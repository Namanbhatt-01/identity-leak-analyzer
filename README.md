...
## 🔮 Future Roadmap (NCIIPC Critical Infrastructure Protections)

To align this framework with national security requirements for defending India's Critical Information Infrastructure (CII), the following modules are planned for integration:

1. **Ingestion of Industrial Control & SCADA Telemetry (OT Protection)**
   * **Goal**: Expand the parser engine to consume network telemetry logs from industrial operational technology (OT) protocols (e.g., Modbus TCP, DNP3, and IEC 60870-5-104) to monitor for unauthorized transaction attempts or command anomalies on physical infrastructure controllers.
2. **Standardized Threat Intelligence Ingestion (STIX / TAXII 2.1)**
   * **Goal**: Replace the custom JSON schema parser with a robust structural C++ pipeline that handles STIX (Structured Threat Information Expression) objects, enabling immediate compatibility with automated threat feeds generated by national defense agencies.
3. **Native Yara/Sigma Signature Engine Integration**
   * **Goal**: Build a lightweight, native C++ matching module to process simplified Yara syntax (for file/memory audits) and Sigma rules (for log events) to identify specific infrastructure-targeting nation-state threat group tactics.
4. **Deterministic Rate Limiting (DoS/DDoS Mitigation)**
   * **Goal**: Upgrade the thread-safe `RateLimiter` component from simple sleep timers to a production-grade **Token Bucket** or **Leaky Bucket** algorithm to protect public service databases from resource-exhaustion and Denial of Service (DoS) attempts.
